...
const HistoryCorrection = () => {
  ...
  const onSearchClick = async () => {
    ...
    try {
      const result = await axios.post(
        variables.VIO_API_URL + `/get-hist-correction-count`,
        {
          period_code: quarter === null ? null : quarter.value,
          period_year: year === null ? null : year.value,
          inn: inn === "" ? null : inn,
          date_start: date_start === "" || date_start === null ? null : date_start.toISOString().replace('000Z', '300Z'),
          date_end: date_end === "" || date_end === null ? null : date_end.toISOString().replace('000Z', '300Z'),
  ...
  const getServerSideDatasource = (count) => {
    return {
      getRows: async (params) => {
        ...
        try {
          const result = await axios.post(
            variables.VIO_API_URL + `/get-hist-correction-list`,
            {
              period_code: quarter === null ? null : quarter.value,
              period_year: year === null ? null : year.value,
              inn: inn === "" ? null : inn,
  ...
  return (
    <>
      ...
      />
      <Breadcrumb pageName="История коррекции реестра нарушителей" />
