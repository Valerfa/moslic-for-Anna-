...
const EmailingJournal = ({token, personlog_id, resources}) => {
  ...
  const [DataColumns] = useState(
      [
                  ...
                  },
                  {
                    enableValue: true,
                    field: "doctypename",
                    headerName: "Тип рассылаемого документа",
                    resizable: true,
                    sortable: true,
                    filter: null,
                  }
  ...
  const getFiltersResults = async () => {
    setLoad(true);
    const data = await getFiltersLists('options', 'RequestEmailedDocType', token);
  ...
  const onSearchClick = async() => {
    ...
    try {
      const result = await axios.post(
        variables.API_URL + `/Api/EmailingJournal/count?personlog_id=${personlog_id}`,
        activeDataFilters,
        {
          headers: {
  ...
  const getServerSideDatasource = (count) => {
    return {
      getRows: async (params) => {
        ...
        try {
          const result = await axios.post(
            variables.API_URL + `/Api/EmailingJournal/export?personlog_id=${personlog_id}`,
  ...
  const detailCellRendererParams = useMemo<any>(() => {
    return {
      ...
      getDetailRowData: async (params) => {
        ...
        try {
          const result = await axios.post(
                variables.API_URL + `/Api/EmailingJournal/export_log?personlog_id=${personlog_id}`,
...
export default EmailingJournal;
