...
const DistributionList = ({token, personlog_id, resources}) => {
  ...
  const [DataColumns] = useState(
      [
          ...
          },
          {
            enableValue: true,
            field: "id_person",
            headerName: "Назначить",
            resizable: true,
            sortable: true,
            filter: null,
            cellRenderer: (params) => {
              console.log(params.getValue(), persons[params.getValue()])
              try {
                if (persons[params.getValue()] === undefined)
                  return params.getValue();
                return persons[params.getValue()];
              } catch (e) {
                console.log(e)
                return params.getValue()
              }
  ...
  const getPersons = async() => {
    persons = {}
    for (const person of await getFiltersLists('options', `CurrentDivisionPerson&person_id=${personlog_id}`, token)) {
  ...
  const getFiltersData = async() => {
    ...
    return {
      FilterExt: {
        ...
        sids1: [1],
        // Вернутся к сидам и к реализации их на стороне пользователя
      ...filters
    }
  }
  ...
  const onSearchClick = async() => {
    ...
    try {
      const result = await axios.post(
        variables.API_URL + `/Api/DistributionList/count?person_id=${personlog_id}`,
        activeDataFilters,
        {
          headers: {
  ...
  const getServerSideDatasource = (count) => {
    return {
      getRows: async (params) => {
        ...
        try {
          const result = await axios.post(
            variables.API_URL + `/Api/DistributionList/export?personlog_id=${personlog_id}`,
  ...
  const Download = async () => {
    ...
    try {
      const result = await axios.post(
        variables.API_URL + `/Api/DistributionList/export_to_excel?personlog_id=${personlog_id}`,
        {
          ...activeDataFilters,
  ...
  return (
    <>
      ...
      />
      <Breadcrumb pageName="Распределение дел по исполнителям" />
      <div className="grid grid-cols-12 gap-4 md:mt-6 md:gap-6 2xl:mt-7.5 2xl:gap-7.5">
        <div className="col-span-12">
          <CardTable
            onFiltersClick={openFilters}
            name="Распределение дел по исполнителям"
            buttons={
              <>
                <IconButtonDownload
                    onClick={() => Download()}
