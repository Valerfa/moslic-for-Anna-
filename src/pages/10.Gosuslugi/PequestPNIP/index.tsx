...
const RequestPNIP = ({token, personlog_id, resources}) => {
  ...
  const onSearchClick = async() => {
    ...
    try {
      ...
      const result = await axios.post(
        variables.API_URL + `/Api/PequestPNIP/count?personlog_id=${personlog_id}`,
        activeDataFilters,
        {
          headers: {
  ...
  const getServerSideDatasource = (count) => {
    return {
      getRows: async (params) => {
        ...
        try {
          const result = await axios.post(
            variables.API_URL + `/Api/PequestPNIP/export?personlog_id=${personlog_id}`,
  ...
  return (
    <>
      ...
      />
      <Breadcrumb pageName="Запросы в РНИП" />
