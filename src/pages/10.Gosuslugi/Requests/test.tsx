...
const Gosuslugi = () => {
  ...
  const getFiltersResults = async () => {
    const data = await getFiltersLists('requests', 'RequestDecisionDocType', token)
  ...
  const onSearchClick = async() => {
    ...
    try {
      const result = await axios.post(
        variables.API_URL + `/Api/Requests/export_full`,
        activeDataFilters,
        {
          headers: {
  ...
  const Download = async () => {
    ...
    try {
      const result = await axios.post(
        variables.API_URL + `/Api/Requests/export_to_excel`,
        {
          ...activeDataFilters,
