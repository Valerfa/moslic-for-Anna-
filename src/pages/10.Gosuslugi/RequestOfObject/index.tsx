...
const RequestOfObbject = ({token, personlog_id, resources}) => {
  ...
  const onSearchClick = async() => {
    ...
    try {
      ...
      const result = await axios.post(
        variables.API_URL + `/Api/RequestOfObject/count?personlog_id=${personlog_id}`,
        activeDataFilters,
        {
          headers: {
  ...
  const getServerSideDatasource = (count) => {
    return {
      getRows: async (params) => {
        ...
        try {
          const result = await axios.post(
            variables.API_URL + `/Api/RequestOfObject/export?personlog_id=${personlog_id}`,
  ...
  return (
    <>
      ...
      />
      <Breadcrumb pageName="Запросы по объекту" />
...
export default RequestOfObbject;