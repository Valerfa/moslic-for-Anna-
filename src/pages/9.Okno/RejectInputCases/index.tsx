...
const InputCases = ({ token, personlog_id, resources }) => {
  ...
  const onSearchClick = async () => {
    ...
    try {
      const result = await axios.post(
        variables.API_URL +
          `/Api/RejectInputCases/count_docs_list?personlog_id=${personlog_id}`,
        activeDataFilters,
        {
          headers: {
    ...
    return {
      getRows: async (params) => {
        ...
        try {
          const result = await axios.post(
            variables.API_URL +
              `/Api/RejectInputCases/export_docs_list?personlog_id=${personlog_id}`,
    ...
    return {
        ...
        detailCellRendererParams: {
          ...
          getDetailRowData: async (params) => {
            ...
            try {
              const result = await axios.post(
                variables.API_URL +
                  `/Api/RejectInputCases/export_documents?personlog_id=${personlog_id}`,
                {
                  ID_LIST:
      ...
      getDetailRowData: async (params) => {
        ...
        try {
          const result = await axios.post(
            variables.API_URL +
              `/Api/RejectInputCases/export_trans_docs?personlog_id=${personlog_id}`,
            {
              ID_LISTOFDOCLIST:
  ...
  return (
    <>
      ...
      />
      <Breadcrumb pageName="Непринятые входящие дела" />
      <div className="grid grid-cols-12 gap-4 md:mt-6 md:gap-6 2xl:mt-7.5 2xl:gap-7.5">
        <div className="col-span-12">
          <CardTable
            onFiltersClick={openFilters}
            name="Непринятые входящие дела"
            buttons={
              <>
                <button
                  type="button"
