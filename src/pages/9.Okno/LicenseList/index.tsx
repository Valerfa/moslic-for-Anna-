...
const LicenseList = ({ token, personlog_id, resources }) => {
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/okno/license-list")
      onSetTitle("Список лицензий");
    else onSetTitle("Оперативный список лицензий");
    console.log(title);
    setLoad(false);
  }, [location]);
  ...
  const onSearchClick = async () => {
    ...
    try {
      ...
      let apiUrl = "";
      if (title === "Список лицензий")
        apiUrl = `/Api/LicenseList/count?personlog_id=${personlog_id}`;
      else apiUrl = `/Api/OperLicenseList/count?personlog_id=${personlog_id}`;
      console.log(apiUrl);
  ...
  const getServerSideDatasource = (count) => {
    return {
      getRows: async (params) => {
        ...
        let apiUrl = "";
        if (title === "Список лицензий")
          apiUrl = `/Api/LicenseList/export?personlog_id=${personlog_id}`;
        else
          apiUrl = `/Api/OperLicenseList/export?personlog_id=${personlog_id}`;
        console.log(apiUrl);
        try {
          const result = await axios.post(
            variables.API_URL + apiUrl,
            ...
            buttons={
              <>
                ...
                <Link to="/subject-view" relative="path">
                  <IconButtonWatch title="Просмотр субъекта"></IconButtonWatch>
                </Link>
              </>
            }>
            <>
