...
const DropdownUser = ({setToken, token}) => {
  ...
  useEffect(() => {
    if (token !== null && token !== undefined) {
      setUsername(token.username === undefined ? '' : token.username)
      setWorkspace(token.workspace === undefined  || token.workspace === null ? '' : token.workspace.name)
    }
  }, [token]);
    ...
    <div className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">{workspace}</span>
        </span>
      ...
      >
        ...
        <button className="flex items-center gap-3.5 py-4 px-6 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base" onClick={() => setToken(null)}>
          <svg
            className="fill-current"
            width="22"
            height="22"
